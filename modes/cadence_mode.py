...
class CadenceMode(ChordModeBase):
    ...
    def handle_keyboard_input(self, char):
        """Gère les entrées clavier spécifiques au mode cadence"""
        if not char:
            return None
...
#        self.first_display_done = True
    ...
    def run(self):
        ...
        while not self.exit_flag:
            ...
            while prog_index < len(progression_accords) and not self.exit_flag and not skip_progression:
                ...
                if result == 'quit':
                    ...
                    # Rejouer la progression et recommencer
                    play_progression_sequence(self.outport, progression_accords, self.chord_set)
                    prog_index = 0
                    progression_correct_count = 0
                elif result == 'next':
            ...
            # Gestion de fin de progression
            if not self.exit_flag and not skip_progression:
                choice = Prompt.ask("\nProgression terminée ! Appuyez sur Entrée pour la suivante ou 'q' pour revenir au menu principal...", 
                                  console=self.console, choices=["", "q"], show_choices=False)
                if choice == 'q':
                    self.exit_flag = True
                    break
        ...
        # Affichage des statistiques finales (utilise les compteurs de la classe mère)
        self.display_final_stats()


def cadence_mode(inport, outport, play_progression_before_start, chord_set):
    """Point d'entrée pour le mode cadence (compatibilité avec l'ancien code)"""
    mode = CadenceMode(inport, outport, play_progression_before_start, chord_set)
    mode.run()