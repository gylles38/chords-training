...
def is_enharmonic_match_improved(played_chord_name, target_chord_name):
    """
    Version améliorée qui compare les classes de hauteur ET utilise la carte enharmonique.
    Accède directement aux dictionnaires 'all_chords' et 'enharmonic_map' du module.
    """
    # Cas 1: Les noms sont identiques
    if played_chord_name == target_chord_name:
        return True
    
    # Vérification que les accords existent bien dans notre dictionnaire de référence
    if not played_chord_name or not target_chord_name:
        return False
    if played_chord_name not in all_chords or target_chord_name not in all_chords:
        return False
    
    # Cas 2: Vérifier la correspondance via la carte des équivalences enharmoniques
    # Cela permet de gérer les cas comme "Do dièse Majeur" et "Ré bémol Majeur"
    enharmonic_equivalents = enharmonic_map.get(played_chord_name, [])
    if target_chord_name in enharmonic_equivalents:
        return True
    
    # Cas 3 (sécurité): Comparer les classes de hauteur (notes sans l'octave)
    # C'est une méthode de secours robuste si une équivalence n'est pas dans la carte.
    played_pitch_classes = {note % 12 for note in all_chords[played_chord_name]}
    target_pitch_classes = {note % 12 for note in all_chords[target_chord_name]}
